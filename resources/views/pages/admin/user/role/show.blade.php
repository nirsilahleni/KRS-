    ...
    <form action="{{ route('users.role.permissions', $role->id) }}" method="post" id="edit-permission_form">
            ...
            <div class="card bg-danger mb-xl-8">
              <div class="card-body">
                ...
                <div class="form-check form-switch mb-3">
                  <input class="form-check-input switch-checkbox" data-parent-id="{{ $menu->parent_id }}" type="checkbox"
                    role="switch" id="switch-{{ $menu->id }}" data-id="{{ $menu->id }}"
                    name="{{ $menu->module }}" @if (in_array($menu->module, $permissions)) checked @endif>
                </div>
                @php
                ...
                @foreach ($access as $item)
                  <div class="pt-3 form-check">
                    <input type="checkbox" class="form-check-input related-checkbox" name="{{ $item->module }}"
                      data-parent="{{ $menu->id }}" id="checkbox-{{ $item->module }}"
      ...
      <div class="d-flex justify-content-between position-sticky" style="bottom: 2em; right:2em;">
        <div class="d-flex gap-2">
          ...
          <button class="btn btn-success" id="check-all_btn">
            <i class="fal fa-check"></i>
            <span class="ms-2">Check All</span>
          </button>
        </div>
        <div class="d-flex">
          <a href="{{ route('users.role.index') }}" class="btn btn-light" cancel-btn>Kembali</a>
    ...
    <script>
      $(document).ready(function() {
        $('.switch-checkbox').change(function() {
          ...
          if ($(this).is(':checked')) {
            $('.switch-checkbox[data-parent-id="' + id + '"]').prop('checked', true).trigger('change');
            $('.related-checkbox[data-parent="' + id + '"]').prop('checked', true);
          } else {
            $('.switch-checkbox[data-parent-id="' + id + '"]').prop('checked', false).trigger('change');
        ...
        $('#check-all_btn').on('click', function(ev) {
            ev.preventDefault();
            ev.stopPropagation();
          $('.switch-checkbox').prop('checked', true).trigger('change');
        });
