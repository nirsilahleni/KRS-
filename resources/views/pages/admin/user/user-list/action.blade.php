<div class="d-flex justify-content-center align-items-center gap-2">
    ...
    @can($globalModule['update'])
        <button data-action="edit" data-target="#edit-user_modal" data-url="{{ route('users.user-list.edit', $user->id) }}"
    ...
    <ul class="dropdown-menu">
        ...
        <li>
            @can($globalModule['delete'])
                <button data-url="{{ route('users.user-list.destroy', $user->id) }}" data-action="delete"
                    data-table-id="userlist-table" data-name="{{ $user->name }}" class="btn w-100 text-start">
