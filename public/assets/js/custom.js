    ...
    const fields = $(selector)
        .find(
            `input:not([type="submit"])${
                required ? "[required]" : ""
            }:not(.ck-hidden), textarea${required ? "[required]" : ""}, select${
                required ? "[required]" : ""
            }`
        )
...
function handleAjaxError(jqXhr, callback = () => {}) {
    ...
    } else if (jqXhr.status >= 400 && jqXhr.status < 500 && jqXhr.status != 422) {
            ...
            new Dropzone(`#${id}`, {
                ...
                init: function () {
                    ...
                    this.on("addedfiles", function (files) {
                        $(document).trigger(`dropzone:addedfiles-${id}`, [files]);
                    })
            ...
            $.ajax({
                ...
                error: function (jqXhr) {
                    handleAjaxError(jqXhr, () => {
                        ...
                        const firstError = currentForm.find(".is-invalid").first();
                        ...
                        $(document).trigger(`form-error:${currentForm.attr("id")}`, jqXhr.responseJSON);
                    });
                },
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (actionUrl) {
                    ...
                    $(document).trigger("action-cofirmed:" + buttonId);
                }
            }
        });
    });
