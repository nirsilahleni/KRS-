...
class SlideShowItemController extends Controller
{
    ...
    {
        $slideshow->items()->create($request->validated());
        return ResponseFormatter::created('Slide Item berhasil ditambahkan');
    }
    ...
    {
        return response()->json([...collect($item->toArray())->except('image'), 'image' => getFileInfo($item->image)]);
    }
